...
const boardUpdateDebounceDelay = 100 // debounce flickering of the board
...
export class GameHandler {
  ...
  public async updateArduinoBoard(newBoard: Array<Array<SquareState>>) {
    ...
    this.debouncedBoardUpdate = {
      board: newBoard,
      receivedAt: Date.now(),
    }

    // requires the board to be stable for 100ms before updating the board
    setTimeout(() => {
      if (
        Date.now() - this.debouncedBoardUpdate.receivedAt >
        boardUpdateDebounceDelay
      ) {
        this.arduinoBoard = this.debouncedBoardUpdate.board
        this.reconcile()
      }
    }, boardUpdateDebounceDelay)
  }
